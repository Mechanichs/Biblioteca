...
int botoes[] = {8, 9, 6, 7};
bool estadoBotao;
bool estadoBotao#include <EEPROM.h>
#include <LiquidCrystal.h>
...
#define slot_limite1 1
#define slot_limite2 2
#define slot_limite3 3
#define slot_limite4 4

int           sensores[NUM_SENSORES] = {A2/*, A3, A5, A6*/}; // Sensores ligados às portas analógicas
int           verificadores[NUM_SENSORES] = {A0/*, A4, A1, A0*/};  // Resposansaveis por gravar saida do potenciometro
int           limite_POT[NUM_SENSORES] = {EEPROM.read(1)/*, 554, 554, 554*/};  // Variável responsável por definir o limiar do potenciometro medido analogicamente em relação à sensibilidade do sensor
bool          flag_calibracao[NUM_SENSORES];
int           nivel                  = 0;    // Variável responsável pelo nível de ruído
int           endereco               = 0;    // Endereço de memória que vai armazenar quantidade de vezes que a sirene acionou
int           Q_Acionamento          = 0;    // Variável responsável por armazenar quantidade de vezes que a sirene acionou  
...
void alterarlimiares()
{
  ...
  do{
    if(digitalRead(botoes[0]) == HIGH){
      ...
      EEPROM.write(slot_limite1, alteradorDePotenciometro);
      break;
   }  
   if(digitalRead(botoes[1]) == HIGH){
      ...
      EEPROM.write(slot_limite2, alteradorDePotenciometro);
    }
    if(digitalRead(botoes[2]) == HIGH){
      ...
      EEPROM.write(slot_limite3, alteradorDePotenciometro);
    }
